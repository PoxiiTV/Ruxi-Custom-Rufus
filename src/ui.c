...
void UpdateProgress(int op, float percent)
{
	...
	// Headless mode: emit JSON progress to stdout for Ruxi frontend
	if (bHeadless) {
		static uint64_t LastHeadlessRefresh = 0;
		if (GetTickCount64() > LastHeadlessRefresh + 500) {
			LastHeadlessRefresh = GetTickCount64();
			printf("{\"status\":\"progress\",\"percent\":%d}\n", (pos * 100) / MAX_PROGRESS);
			fflush(stdout);
		}
	}

	// Reduce the refresh rate, to avoid weird effects on the sliding part of progress bar
	if (!bHeadless && GetTickCount64() > LastRefresh + (2 * MAX_REFRESH)) {
