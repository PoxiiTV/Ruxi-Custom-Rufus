	...
	if (boot_type == BT_IMAGE) {
		...
		if ((WindowsVersion.Version >= WINDOWS_8) && IS_WINDOWS_1X(img_report) && (!is_windows_to_go)) {
			if (img_report.has_panther_unattend) {
				...
				if (bHeadless) {
					// Ruxi: skip dialog, apply optimal settings automatically
					StrArrayDestroy(&selection.choices);
					StrArrayDestroy(&selection.tooltips);
					int headless_flags = UNATTEND_SECUREBOOT_TPM_MINRAM | UNATTEND_NO_ONLINE_ACCOUNT |
...
//				WriteSettingBool(SETTING_ENABLE_RUNTIME_VALIDATION, validate_md5sum);
	...
	case UM_HEADLESS_INIT: {
		...
		target_type = TT_UEFI;
		// Set filesystem to NTFS
		selected_fs = FS_NTFS;
		// Configure Windows User Experience flags: bypass TPM/SecureBoot/RAM,
		...
		for (idx = 0; idx < ComboBox_GetCount(hDeviceList); idx++) {
			...
			ComboBox_GetLBTextU(hDeviceList, idx, drive_label);
			if (headless_device[0] != 0 && strchr(drive_label, headless_device[0]) != NULL) {
		...
		if (argv != NULL) {
			...
			while ((opt = getopt_long(argc, argv, "ghxf:i:l:w:z:", long_options, &option_index)) != EOF) {
				...
				case 'H':
					...
					break;
				case 'D':
					safe_strcpy(headless_device, sizeof(headless_device), optarg);
					break;
				case 'U':
